...
export default function ForgetPassStep1() {
...
function ForgetPasswordStep1Component() {
    ...
    useEffect(() => {
        if (sp.get("lang") === "en") {
            initializeTitlesWithValue("Reset Pass")
        } else if (sp.get("lang") === "fa") {
            initializeTitlesWithValue("فراموشی رمز عبور")
        }
                    ...
                    for (let key in states) {
                        if (states[key].name === "IsReadyToSetPass") {
                            if (states[key].value === true) {
                                history.push({
                                    pathname: getRoutesItems().resetPasswordStep2.route,
